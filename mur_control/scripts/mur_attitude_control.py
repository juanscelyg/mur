...
class MURAttitudeControlNode:
    def __init__(self):
        ...
        self.error_relative = np.zeros(shape=(2,1))
    ...
    def cmd_desired_callback(self,msg):
        if self.state == 'POSCTL':
            self.roll_d = self.pid_x.controlate(self.error_relative[0,],-self.vita[0,],self.t)
            self.pitch_d = self.pid_y.controlate(self.error_relative[1,],-self.vita[1,],self.t)
    ...
    def get_errors(self):
        ...
        rot_mat=mur_common.rot2(self.nita[5,])
        rot2d_mat = np.transpose(rot_mat)
        self.error_relative = np.matmul(rot2d_mat,np.array([[self.error_pos[0,]],[self.error_pos[1,]]]))
        #rospy.loginfo("Error rel:= %s " %self.error_relative)
    ...
    def force_callback(self):
        ...
        if self.state == 'STABILIZED':
            Tz = self.pid_z.controlate(self.error_pos[2,],-self.vita[2,],self.t) + self.g_z
            ...
            Tz = self.pid_z.controlate(self.error_pos[2,],-self.vita[2,],self.t) + self.g_z
            T_p = self.pid_pitch.controlate(self.error_pos[3,],-self.vita[3,],self.t)
            T_r = self.pid_roll.controlate(self.error_pos[4,],-self.vita[4,],self.t)
            T_y = 0.0 #self.pid_yaw.controlate(self.error_pos[5,],-self.vita[5,],self.t)
